            ...
            if (typeof plugin.all === 'function') {
                try {
                    ...
                    for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                        ...
                        if (data.exists)
                                    m.reply(`*[ ⚠️ مرحبا سيدي ناتسو  هذا الامر خاطئ ⚠️ ]*\n\n*—◉ الخطأ:* ${m.plugin}\n*—◉ المستخدم:* ${m.sender}\n*—◉ الامر:* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\`\n\n*[❗] قم بابلاغ المطور ليعطيك حلا لاستخدام رقم المطور 201554824764 الامر #reporte*`.trim(), data.jid)
                    }
                }
            }
            if (!opts['restrict'])
...
if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
  ...
  user.commandCount = 1
        ...
        if (!m.fromMem && m.text.match(/حوده|hoda|@201554824764|بوت|bot|ناتسو|Natsu/gi)) {
        let emot = pickRandom(["🎃", "❤", "😘", "😍", "💕", "😎", "🙌", "⭐", "👻", "🔥","🤖"," ✨","🌹","🩵","🌙"])
